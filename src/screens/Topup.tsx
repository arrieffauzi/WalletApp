...
const Topup = () => {
  ...
  const handleOnchangeText = (value: string, fieldName: string) => {
    ...
    setTopUpData({...topUpData, [fieldName]: parseInt(value)});
  };

  const onSubmit = () => {
    let contextData = user.user;
    axios
      .post('https://tht-api.nutech-integrasi.app/topup', topUpData, {
        headers: {
  ...
  return (
    <View style={styles.background}>
      <View style={styles.page}>
        <TitleHeader title="Top Up" backButton={true} />
        <View style={styles.container}>
          ...
          {!isValid && (
            <Text style={styles.errorMsg}>Minimum Topup is Rp.10.000</Text>
          )}
        </View>
        <Button textButton="Top Up" onPress={onSubmit} disabled={!isValid} />
      </View>
    </View>
  );
};
